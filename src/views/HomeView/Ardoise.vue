...
const refreshData = async () => {
  ...
  try {
    ...
    EventBus.emit('showToast', {
      type: 'error',
      message: t('orders.dashboard.messages.error_loading')
    })
  } finally {
...
const takeOrder = async (orderCode: string) => {
  ...
  try {
    ...
    // Refresh for synchronization
    await refreshData()
    
    EventBus.emit('showToast', {
      type: 'success',
      message: t('orders.dashboard.messages.order_taken', { code: orderCode })
    })
  } catch (error) {
    // On error, revert and refresh
    await refreshData()
    EventBus.emit('showToast', {
      type: 'error',
      message: t('orders.dashboard.messages.error_taking')
    })
  }
}
...
const updateStatus = async (orderCode: string, newStatus: 'COMPLETED' | 'CANCELED') => {
  ...
  try {
    ...
    EventBus.emit('showToast', {
      type: 'success',
      message: t('orders.dashboard.messages.status_updated', { code: orderCode })
    })
  } catch (error) {
    // On error, revert and refresh
    await refreshData()
    EventBus.emit('showToast', {
      type: 'error',
      message: t('orders.dashboard.messages.error_updating')
    })
  }
}
...
<template>
  <div class="container mx-auto px-2 sm:px-4 py-2 h-full min-h-screen">
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      <StatCard 
        :title="t('orders.dashboard.stats.pending')" 
        :value="counts.draft" 
        icon="⏳" 
      ...
      <StatCard 
        :title="t('orders.dashboard.stats.in_progress')" 
      ...
      <StatCard 
        :title="t('orders.dashboard.stats.completed')" 
      ...
      <StatCard 
        :title="t('orders.dashboard.stats.canceled')" 
    ...
    <h1 class="text-xl md:text-2xl font-bold mb-4 text-center lg:hidden">{{ t('orders.dashboard.title') }}</h1>
    
    <div class="flex flex-col xl:flex-row gap-4 h-full">
      <!-- Available orders column (DRAFT) -->
      <div class="flex-1 order-2 xl:order-1 min-h-[300px] xl:min-h-[calc(100vh-100px)] overflow-y-auto">
        <div class="bg-white rounded-lg shadow p-3 sm:p-4 h-full">
          <h2 class="text-lg sm:text-xl font-bold mb-3 sm:mb-4 text-orange-600">{{ t('orders.dashboard.sections.new_orders') }}</h2>
          
          <div v-if="availableOrders.length === 0" class="text-gray-500 text-center py-8 text-sm sm:text-base">
            {{ t('orders.dashboard.sections.no_orders') }}
          </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 2xl:grid-cols-2 gap-2 sm:gap-3">
            ...
            >
              <div class="flex justify-between items-center mb-1 sm:mb-2">
                <h3 class="font-semibold text-xs sm:text-sm md:text-base">{{ t('orders.dashboard.order_number', { code: order.Code }) }}</h3>
              </div>
              ...
              <div class="flex justify-end pt-2 border-t">
                <button
                  ...
                  :title="t('orders.dashboard.actions.take_order')"
                >
      ...
      <!-- Orders in progress column (IN PROGRESS) -->
      <div class="flex-1 order-1 xl:order-2 min-h-[300px] xl:min-h-[calc(100vh-100px)] overflow-y-auto">
        <div class="bg-white rounded-lg shadow p-3 sm:p-4 h-full">
          <h2 class="text-lg sm:text-xl font-bold mb-3 sm:mb-4 text-blue-600">{{ t('orders.dashboard.sections.in_progress') }}</h2>
          
          <div v-if="takenOrders.length === 0" class="text-gray-500 text-center py-8 text-sm sm:text-base">
            {{ t('orders.dashboard.sections.no_in_progress') }}
          </div>
            ...
            >
              ...
              <div class="flex justify-end space-x-1 sm:space-x-2 pt-2 border-t">
                <button
                  ...
                  :title="t('orders.dashboard.actions.mark_completed')"
                />
                <button
                  ...
                  :title="t('orders.dashboard.actions.cancel_order')"
                />
              </div>
            </div>
          </div>
