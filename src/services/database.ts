...
import type { PlateOption, Content, PlateContent, Compositions, composition, Categorieplat, Stocks, Table, Team, StocksExit } from "./serviceInterface";
...
export const createNewExit = async (payload:StocksExit[], _token:string,restaurantCode:string) => {
    const url =  `${import.meta.env.VITE_APP_STOCKS_BASE_URL_V}/Entries/Remove?RestaurantCode=${restaurantCode}`;
    try {
        const response = await axios.post(url, { 
            entries:payload
        }, {
        ...
        console.error('database.createnewexit', response.data)
        return response.data;
    } catch (error) {
        console.error('Database.createNewExit.error ::', error);
        throw error;
    }        
}
     
export const fetchStocks = async (_token:string,restaurantCode:string) => {
    const url =  `${import.meta.env.VITE_APP_STOCKS_BASE_URL_V1}/Dashboard?RestaurantCode=${restaurantCode}&dashboard=false&entries=true`;
    const authStore = useAuthStore()
    try {
        const response = await axios (url, {
            headers: {
...
export const updateStocksExit = async (payload:any[], _token:string,restaurantCode:string) => {
    const url =  `${import.meta.env.VITE_APP_STOCKS_BASE_URL_V1}/Entries/Remove?RestaurantCode=${restaurantCode}`;
    const authStore = useAuthStore()
    try {
        const response = await axios.put(url, { 
            entries: payload
        ...
        console.error('database.updateStocksExit', response.data.body)
        return response.data;
    } catch (error) {
        console.error('Database.updateStocksExit.error ::', error);
        throw error;
    }
}
export const deleteStocksExit = async (Code: any[], _token:string, restaurantCode:string) => {
    const url =  `${import.meta.env.VITE_APP_STOCKS_BASE_URL_V1}/Entries/Remove?RestaurantCode=${restaurantCode}`;
    const authStore = useAuthStore()
    try {
        const response = await axios.delete(url,{
...
export const fetchpendingStocks = async (_token:string,restaurantCode:string) => {
    const url =  `${import.meta.env.VITE_APP_STOCKS_BASE_URL_V1}/Dashboard/Draft?RestaurantCode=${restaurantCode}`;
    const authStore = useAuthStore()
    try {
        const response = await axios (url, {
            headers: {
        ...
        console.error('Database.fetchstockspending.error ::', error);
        throw error;
    }
}
//Table
